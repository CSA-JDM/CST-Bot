...
async def on_message(msg):
	if msg.author.id == "118845112599052288" and msg.content.startswith("!calc"):
		...
		print("Live for " + ("{}:{}:{}:{} (d:h:m:s)".format(upday, uphour, upminute, upsecond) if uptime > day else ("{}:{}:{} (h:m:s)".format(uphour, upminute, upsecond) if uptime>hour else ("{}:{} (m:s)".format(upminute, upsecond) if uptime>minute else "{} seconds".format(upsecond)))) + "\n")
		await client.send_message(msg.channel, "live for " + ("{}:{}:{}:{} (d:h:m:s)".format(upday, uphour, upminute, upsecond) if uptime > day else ("{}:{}:{} (h:m:s)".format(uphour, upminute, upsecond) if uptime>hour else ("{}:{} (m:s)".format(upminute, upsecond) if uptime>minute else "{} seconds".format(upsecond)))))
		...
		for x in expression:
			if str(x) not in "0123456789abcdefintx+-=<>~^*/%&|()[].,\"' \r\n\x00":
				await client.send_message(msg.channel, "Characters allowed are 0123456789abcdefintx+-=<>~^*/%&|()[].,\"' and spaces")
				return
		try:
    ...
    if dcount == 1:
        try:
            ...
            if splitted[0] != '' and type(int(splitted[0])) is int and type(int(splitted[1])) is int:
                ...
                return msg.author.name + ", rolling " + splitted[1] + " sided dice " + splitted[0] + (" time: " if int(splitted[0]) == 1 else " times: ") + "\n" + str(sum(roll)) + " (" + '+'.join([str(x) for x in roll]) + ")"
            elif splitted[0] == '' and type(int(splitted[1])) is int:
                return msg.author.name + ", rolling " + splitted[1] + " sided dice 1 time: " + str(randint(1, int(splitted[1])))
            else:
